###UPDATED STATSBOMB EXPLORATION ANALYSIS###
rm(list=ls())
library(StatsBombR)
library(ggplot2)
library(dplyr)
...
#Get World Cup 2022 Data (Men's)
matches_WC22 <- Matches %>%
  filter(competition.competition_id == 43, season.season_id == 106)

#View the filtered data
print(head(matches_WC22))

#Extract unique match IDs for World Cup 2022
match_ids_WC22 <- unique(matches_WC22$match_id)

#Print the unique match IDs
print(match_ids_WC22)

#Get World Cup 2023 Data (Women's)
matches_WC23 <- Matches %>%
  filter(competition.competition_id == 72, season.season_id == 107)

#View the filtered data
print(head(matches_WC23))

#Extract unique match IDs for World Cup 2022
match_ids_WC23 <- unique(matches_WC23$match_id)

#Print the unique match IDs
print(match_ids_WC23)
...
#Create an empty list to store results from each match
team_features_list <- list()

#Loop through each match in matches_WC23
#Create an empty list to store results from each match
team_features_list_w <- list()

#Loop through each match in matches_WC23
for (i in 1:nrow(matches_WC23)) {
  events <- get.matchFree(matches_WC23[i, ])
  
  #Extract the match ID
  match_id <- as.character(matches_WC23[i, "match_id"])
  
  #Extract home and away teams from the matches_WC22 dataframe
  home_team <- matches_WC23[i, "home_team.home_team_name"]
  away_team <- matches_WC23[i, "away_team.away_team_name"]
...
#Loop through each match in matches_WC22
for (i in 1:nrow(matches_WC22)) {
  events <- get.matchFree(matches_WC22[i, ])
  
  #Extract the match ID
  match_id <- as.character(matches_WC22[i, "match_id"])
  
  #Extract home and away teams from the matches_WC22 dataframe
  home_team <- matches_WC22[i, "home_team.home_team_name"]
  away_team <- matches_WC22[i, "away_team.away_team_name"]
...
#1 = female, 0 = men
final_features_df$gender <- c(rep("0", 128), rep("1", 128))
...
coef_df <- coef_matrix %>%
  ...
  rename('s' = 's=0.02099019') %>%  #safer way: check the name of the column
...
# #Create a shifted variable for plotting
# top_features <- top_features %>%
#   mutate(
#     odds_ratio_shifted = odds_ratio - 1,  #shift so 1 (neutral) becomes 0
#     direction = odds_ratio > 1
#   )
...
ggplot(top_features, aes(x = reorder(feature, abs_coef), y = abs_coef)) +
  ...
  coord_flip() +
  labs(
    title = "Top Variables Selected for Predicting Men's vs. Women's Match",
    x = "Variable",
...
#Loop through each match for WC2023
for (i in 1:nrow(matches_WC23)) {
  events <- get.matchFree(matches_WC23[i, ])
...
##men
pass_length_list <- list()
#Loop through each match for WC2022
for (i in 1:nrow(matches_WC22)) {
  events <- get.matchFree(matches_WC22[i, ])
...
######Total passes and total successful passes per team (needs to be fixed)




####Revisiting Passing Accuracy, but filtering for only regular passes####
reg_pass_length <- list()
...
for (i in 1:nrow(matches_WC22)) {
  ...
  #Identify successful regular play passes and extract additional fields
  passes <- passes %>%
  ...
  match_pass_acc <- passes %>%
    ...
    summarise(
      ...
      accuracy = successful_passes / total_passes,
      avg_pass_length = mean(pass_length, na.rm = TRUE),
      avg_successful_pass_length = mean(pass_length[successful == 1], na.rm = TRUE),
      .groups = "drop"
    )
  
  
  #Store in list
  reg_pass_length[[i]] <- match_pass_acc
}

#Combine all match results into one data frame
reg_pass_length_m <- bind_rows(reg_pass_length)
reg_pass_length_m$gender <- "Men"
...
# Combine all match results into one data frame
reg_pass_length_w <- bind_rows(reg_pass_length)
reg_pass_length_w$gender <- "Women"

reg_pass_both <- rbind(reg_pass_length_m, reg_pass_length_w)
...
combined_stats_m <- get_match_stats(matches_WC22, "Men")
combined_stats_w <- get_match_stats(matches_WC23, "Women")
...
write.csv(combined_stats, "~/Downloads/combined_stats.csv")

######DATAFRAME TO CALCULATE PASS DIRECTION AND PRESSURE#####
#Initialise list to store pass data - REGULAR PASSES ONLY #
all_passes <- list()
...
##women
#Initialise list to store pass data 
all_passes <- list()
...
passes_combined_w <- passes_combined_w %>%
  select(-c(goalkeeper.penalty_saved_to_post, goalkeeper.success_out, player_off.permanent))

passes_combined_m <- passes_combined_m %>%
  select(-c(half_start.late_video_start))

passes_combined <- rbind(passes_combined_m, passes_combined_w)


######PLOT PASS ACCURACY PER TEAM, BY GENDER#####
...
for (i in 1:nrow(matches_WC22)) {
  ...
  passes <- passes %>%
    mutate(
      time_bucket = floor(minute / 10) * 10,
      successful = ifelse(is.na(pass.outcome.name), 1, 0),
      pass_height = pass.height.name
    )
  
  #calculate passing accuracy per match, time bucket, and pass height
  accuracy_by_time <- passes %>%
    group_by(match_id, time_bucket, pass_height) %>%
    summarise(
      total_passes = n(),
      successful_passes = sum(successful),
      accuracy = successful_passes / total_passes,
...
# Combine all match results into one data frame
acc_over_time_m <- bind_rows(acc_over_time)
acc_over_time_m$gender <- "Men"
...
#Combine all match results into one data frame
acc_over_time_w <- bind_rows(acc_over_time)
acc_over_time_w$gender <- "Women"
...
ggplot(acc_over_time_summary, aes(x = time_bucket, y = accuracy, color = gender)) +
...
ggplot(acc_over_time_both_summary, aes(x = time_bucket, y = accuracy, group = match_id, color = gender)) +
  geom_line(alpha = 0.4) +
  labs(
...
ggplot(pass_length_both, aes(x = gender, y = avg_pass_length, fill = gender)) +
...
t.test(avg_pass_length ~ gender, data = pass_length_both)
...
ggplot(combined_stats, aes(x = pressures_per_min, y = avg_pass_length, color = gender)) +
...
model4 <- glm(accuracy ~ gender * avg_pass_length, data = pass_length_both)
summary(model4)
...
######Calculate pass directions by gender####
...
# Combine all match results into one data frame
pass_angles_w <- bind_rows(pass_angles)
pass_angles_w$gender <- "Women"
...
#Combine all match results into one data frame
pass_angles_m <- bind_rows(pass_angles)
pass_angles_m$gender <- "Men"

pass_angles_combined <- rbind(pass_angles_w, pass_angles_m)
...
passes_combined %>%
  ...
  theme(
    ...
    strip.text.y = element_text(size = 45, angle = 90, face = "bold")
  )

ggsave("~/Downloads/pass_acc_context.pdf", plot = last_plot(), width = 24, height = 16, dpi = 5000)
